...
export const allStorySql = 'SELECT sid,u.nickname as author,title,time,introtext,bodytext,displaystatus,commentstatus FROM stories s INNER JOIN users u ON s.aid = u.uid ORDER BY time DESC'
...
const processAllStories = async (uid, outputFileName) => {
  return await processExport(allStorySql, null, true, outputFileName)
}
...
export { thisStart as start, thisEnd as end, processMyStories, processMyComments, processMyDMs, processAllStories }
