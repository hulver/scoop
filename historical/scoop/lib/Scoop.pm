...
sub initialize {
	...
	warn "Set the apache request object \n" if $DEBUG;
	$self->_set_apache_request();

	warn "# Set up the memcache \n" if $DEBUG;
	# Set up the memcache
	#$self->_set_memcache();
	...
	warn "# Get the request params \n" if $DEBUG;
	# Get the request params
	$self->_set_request_params();
	
	warn "# Set up the CGI object \n" if $DEBUG;
	# Set up the CGI object
	$self->_set_cgi();
	
	warn "# Get server info \n" if $DEBUG;
	# Get server info
	...
	#warn " set the version ifno \n" if $DEBUG;
	# set the version ifno
	$self->_set_version_info();

	warn "# See who's asking \n" if $DEBUG;
	$self->_check_user();
	$self->_check_user();
	
	# Set user perm hash
	warn "# Set user perm hash \n" if $DEBUG;
	$self->_set_perms();
	
	# User pref definitions
	warn "# User pref definitions \n" if $DEBUG;
	$self->_set_pref_items();

	# User prefs, at long last!
	warn "# User prefs, at long last! \n" if $DEBUG;
	$self->_set_prefs();
	...
	warn "# And set the boxes namespace \n" if $DEBUG;
	$self->_set_boxes();
	...
	warn "# Load event property definitions \n" if $DEBUG;
...
sub more_initialize {
	...
	warn "# Set the theme used \n" if $DEBUG;
	$self->_set_theme();

	warn "# Set \$S->{UI}->{BLOCKS} \n" if $DEBUG;
	$self->_set_blocks();

	warn "# Set \$S->{UI}->{MACROS} \n" if $DEBUG;
	$self->_set_macros();
	...
	warn "# Load admin tools \n" if $DEBUG;
	$self->_load_admin_tools();

	warn "# Load hooks \n" if $DEBUG;
	$self->_load_hooks();

	warn "# Init the HTML checker, or reset it \n" if $DEBUG;
...
sub memcache		{ return $_[0]->{MEMCACHE}	}
	...
	if ($flags->{special}) {
		...
		while ($template =~ /%%($keys_re.*?)%%/og) {
			...
			unless ($args[0] && defined($special_keys->{$func})) {
				$return =~ s/%%\Q$fullkey\E%%//;
				warn "Special key <<$fullkey>> called, but not found. Clearing key.\n";
				next;
			}
			warn "Return is $return\n" if ($DEBUG && ($fullkey =~ m/embed_youtube/));
			...
			if ($DEBUG) {
				...
				warn "Key ($fullkey) not found!\n" unless ($return =~ /%%\Q$fullkey\E%%/);
			}

			$return =~ s/%%\Q$fullkey\E%%/$replace/g;
		}
...
This returns an obscufacted IP address, to use when displaying comment IP 
...
sub _get_ip_hash {
	...
	my $hash = $self->{REMOTE_IP};

	#$hash .= crc32($hash . $self->{UI}->{VARS}->{ip_hashkey});
...
sub _get_remote_ip {
	...
	my ($port, $addr) = sockaddr_in($self->apache->connection->local_addr());
	$local = inet_ntoa($addr);

	# we'll only look at the X-Forwarded-For header if the requests
    # comes from our proxy at localhost
    return $self->apache->connection->remote_ip() unless 
	( ( ($self->apache->connection->remote_ip() eq "127.0.0.1") or 
	    ($local eq $self->apache->connection->remote_ip())
	  )
	and ($self->apache->headers_in->{'X-Forwarded-For'}));
...
sub check_for_login {
	...
	if (($uname && $pass) && !$mail) {
		...
		unless ($uid) {
			...
			$message = $login_err;
			my $temp = $S->filter_subject($uname);
			$S->log_activity("invalid_login","$temp","");
	...
	if ($uname && $mail) {
		my $temp = $S->filter_subject($uname);
		$S->log_activity("mail_password","$temp","");
