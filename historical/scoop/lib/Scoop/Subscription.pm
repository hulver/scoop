...
sub sub_save_payment {
	...
	my $q_type    = $S->dbh->quote($type);
	my $convert   = $S->{UI}->{VARS}->{currency_conversion} || 1;

	my ($rv, $sth) = $S->db_insert({
		INTO   => 'subscription_payments',
		COLS   => 'uid, order_id, cost, pay_type, auth_date, final_date, paid, type, conversion',
		VALUES => qq|$uid, $q_oid, $q_total, $q_paytype, NOW(), NOW(), 1, $q_type, $convert|
	});
	$sth->finish();
