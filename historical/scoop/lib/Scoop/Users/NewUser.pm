...
sub new_user {
	...
	if ($tool eq 'writeuser') {
		...
		if ($S->{UI}->{VARS}->{'use_captcha'}) {
			my $privkey = $S->{UI}->{VARS}->{'captcha-private'};
			my $remoteip = $S->filter_subject($S->{REMOTE_IP});
			my $challenge = $S->cgi->param('recaptcha_challenge_field');
			my $response = $S->cgi->param('recaptcha_response_field');
		...
		if ($S->{UI}->{VARS}->{'g-use-captcha'}) {
			warn "(new_user) checking captcha v2 ".$S->cgi->param('g-recaptcha-response') if $DEBUG;
			my $result = $S->check_captchav2($S->cgi->param('g-recaptcha-response'));
		...
		warn "(new_user) error is ".$error if $DEBUG;
...
sub check_captchav2 {
	my $S = shift;
	my $response = shift;
	my $privkey = $S->{UI}->{VARS}->{'g-captcha-secretkey'};
	my $remoteip = $S->filter_subject($S->{REMOTE_IP});
	...
	if ( $resp->is_success ) {
		...
		warn "(new_user) checking captcha v2 response ".$answer if $DEBUG;
