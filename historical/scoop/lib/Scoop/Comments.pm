...
sub comment_dig {
	...
	my $sid = $S->filter_subject($S->{CGI}->param('sid'));
	my $pid = $S->filter_subject($S->{CGI}->param('pid'));
	my $cid = $S->filter_subject($S->{CGI}->param('cid'));
	my $tool = $S->filter_subject($S->{CGI}->param('tool'));
	my $mode = $S->filter_subject($S->{CGI}->param('mode'));
	my $post = $S->filter_subject($S->cgi->param('post'));
	my $preview = $S->filter_subject($S->cgi->param('preview'));
	my $showrate = $S->filter_subject($S->{CGI}->param('showrate'));
	my $check_comment = $S->filter_subject($S->{CGI}->param('pending'));
	...
	if ($tool eq 'delete' && $S->have_perm( 'comment_delete' )) {
		if (($S->{CGI}->param('confirm') eq 'delete') && ($S->{APR}->method() eq 'POST')) {
	...
	if ($tool eq 'remove') {
		warn "In Remove\n";
		warn "comment_remove: $S->have_perm('comment_remove')\n";
		warn "uid: $S->{UID} : comment_uid : " . $S->_get_comment_uid($sid,$cid) ."\n";
		warn "comment_delete_own: $S->have_perm('comment_delete_own')\n";

		if ( $S->have_perm( 'comment_remove' ) || ( ($S->_get_comment_uid($sid,$cid) == $S->{UID}) && $S->have_perm('comment_delete_own') ) ) {
			if (($S->{CGI}->param('confirm') eq 'delete') && ($S->{APR}->method() eq 'POST')) {
	...
	$mode = 'Preview' if ($mode eq 'Spellcheck');
	...
	if ( $tool eq 'post' && $post ) {
		...
		if ($S->have_perm( 'comment_post' ) && !$S->_check_archivestatus($sid)) {
			...
			unless ( $check_subj && ($check_subj =~ /[^\s]+/) ) {
				$err .= qq|
	...
	$S->{PARAMS}->{'mode'} = $mode unless ($S->{CGI}->param('mode') eq 'Spellcheck');
	$mode = 'Preview' if ($S->{CGI}->param('mode') eq 'Spellcheck');
...
sub _confirm_delete_comment {
	my $S = shift;
	my ($sid, $cid, $tool) = @_;
	my $retval = qq~<form name="delete_confirm" method="post" action="%%rootdir%%/comments/$sid/$cid/$tool">%%norm_font%%To actually delete this comment, <input type="submit" value="Click here"><input type="hidden" name="confirm" value="delete"></form>%%norm_font_end%%~;
#a href="%%rootdir%%/comments/$sid/$cid/$tool?confirm=delete">click here</a>.%%norm_font_end%%~;
	return $retval
}
...
sub _remove_comment {
	...
	# First, get the uid of comment poster
	my $uid = $S->_get_comment_uid($sid,$cid);
	
	return unless ( (($S->{UID} == $uid) && $S->have_perm('comment_delete_own')) || ($S->have_perm('comment_remove')) );
	...
	my ($rv, $sth) = $S->db_update({
		ARCHIVE => $S->_check_archivestatus($sid),
		WHAT => 'comments',
		SET => qq|comment = $removed_body, subject = $removed_subject, sig_status = '-1'|,
		WHERE => qq|cid = $q_cid AND sid = $q_sid|});
...
sub display_comments {
	...
	if($commentstatus==-1){	# If comments disabled, just bail out
...
sub get_list {
	...
	foreach my $i (@{$S->{CURRENT_COMMENT_THREAD}->{$pid}}) {
		...
		push (@mark_read,$S->{CURRENT_COMMENT_LIST}->[$i]->{cid}) if $S->{CURRENT_COMMENT_LIST}->[$i]->{cid} >= $highest;;
		if ($dispmode eq 'nested' || $dispmode eq 'flat' || $dispmode eq 'flat_unthread') {
			$list .= $level_start;
			$list .= $S->format_comment($S->{CURRENT_COMMENT_LIST}->[$i], $commentstatus);
			$list .= $level_end;
		} else {
		...
		if ($dispmode ne 'nested' && $dispmode ne 'flat' && $dispmode ne 'flat_unthread') {
		 	my($sublist,@subcids) = $S->get_list($sid, $S->{CURRENT_COMMENT_LIST}->[$i]->{cid}, $commentstatus, $dispmode, $depth+1);
			$list .= $sublist;
			push @cids, (@subcids);
		}
