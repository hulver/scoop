...
sub auto_format {
	...
	#s!([\x80-\xFF])!'&#'.ord($1).';'!ge;
	s!scoop-(.*);!'scoop-scoop;'.$1.';'!ge;

	$_ = $S->ecode_escape($_, 'auto');
	...
	# Escape significant characters preceded by a backslash
	s#\\\\#scoop-slash;#g; # Escape double backslashes first
	s#\\<#\x01#g;
	s#\\>#\x02#g;
	s#\\&#\x03#g;
	s#\\"#\x04#g;
	...
	my $url_regexg = '(?:ttps?|tp|ile)://(?:[^\s<]|\Z)+(?=[\s<]|\Z)';
	my $tag_regex = '<[^><]*?>';

		#$a =~ s#([^a-zA-Z0-9])#chr(ord($1)|0x80)#ge;
	s!($url_regexg|$tag_regex)!
		my $a = $1;
		$a =~ s/([^a-zA-Z0-9<>])/'scoop-#'.ord($1).';'/ge;
		$a
	!ge;
...
sub _auto_linkify_urls {
	...
	my $url_regex = '(?:https?|ftp|file)://[^\s<>]+?';
	my $url_regexg = '(?:https?|ftp|file)://[^\s<>]+'; # greedy
...
sub js_quote {
	...
	$string =~ s#'#\\'#g;
	$string =~ s#"#\\"#g;
	$string =~ s#</(SCRIPT)>#&lt;/$1&gt;#gi;
	$string =~ s#([^\x20-\x7E])#'\\x'.sprintf("%02x",ord($1))#ge;
	return $string;
}

=item * ecode_escape( $text, $postmode )

Escape text enclosed between matching <ecode> tags so that it is not parsed as HTML or as autoformat chars.
Tags match if they are either <ecode></ecode> pairs or <ecode:SomeString></ecode:SomeString>
...
sub ecode_escape {
	...
	my $postmode = shift;
		...
		#$a =~ s!([^a-zA-Z0-9&;<> ])!'&#'.ord($1).';'!ge;
	s!<ecode((?=>)|:\w*)>(.*?)</ecode\1>!
		my $a = $2;
		if ($postmode eq 'auto') {
			$a =~ s#scoop-scoop;#scoop-#g;
		}
		$a = $S->plaintext_format($a);
		$a =~ s/<p>/<br><br>/gi;

		if ($postmode eq 'auto') {
		# Escape potential autoformat chars.  Assume [&;<>] don't need escaping to stop
		# spellchecker from mangling everything; and who in their right mind
		# would re-task HTML special chars for autoformatting?
		$a =~ s/([^a-zA-Z0-9&;<> ])/'scoop-#'.ord($1).';'/ge;
		}
...
=item $S->filter_param($param)

Filters a parameter to remote the possiblity of cross site scripting
