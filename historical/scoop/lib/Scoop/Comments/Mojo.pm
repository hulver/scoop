...
sub calculate_mojo {
	...
	if ($S->{UI}->{VARS}->{mojo_ignore_sections}) {
	        my @sections = split /\s*,\s*/, $S->{UI}->{VARS}->{mojo_ignore_sections};
        	foreach my $w (@sections) {
			my $q_w = $S->{DBH}->quote($w);
			$fetch->{WHERE} .= qq| AND stories.section != $q_w|;
		}
