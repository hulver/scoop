...
sub _comment_rating_type_select {
	my $S = shift;
	my $comment_rating_type = $S->get_comment_option('rating_type');
	my $form = qq|<select name="comment_rating_type" size="1">|;
	$form .= qq|	<option value="1"|;
	$form .= qq| selected="selected"| if ($comment_rating_type eq '1');
	$form .= qq|>Drop down box</option>|;
	$form .= qq|	<option value="2"|;
	$form .= qq| selected="selected"| if ($comment_rating_type eq '2');
	$form .= qq|>Radio Buttons</option>|;
	$form .= "
			</select>
		</td>
	</tr>\n";
...
sub _rating_form {
	...
	my $rate_type = $S->pref('rating_type');
