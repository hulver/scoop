...
sub format_comment {
	...
	if ($comment->{mode} ne 'alone' && $comment->{mode} ne 'Preview') {	
		$replies = ($S->get_list($comment->{sid}, $comment->{cid}, $commentstatus))[0] || '';
	}
	...
	if ($comment->{pid} != 0) {
		...
		if (($comment->{mode} ne 'Preview') && ($S->have_perm('comment_post')) && !$posting_comment && !$commentstatus && $sect_perm_post && !$S->_check_archivestatus($comment->{sid}) ) {
			$comm_options .= qq|\| <a class="post_comment" href="%%rootdir%%/comments/$comment->{sid}/$comment->{cid}/post#here">Reply to This</a> |;
		}
	...
	if ( ($S->have_perm( 'comment_remove' ) || (($S->{UID} == $comment->{uid}) && $S->have_perm('comment_delete_own') ) ) && !$posting_comment) {
...
sub comment_choices_box {
	...
	if ($S->_does_poll_exist($sid)) {
        ...
        my $accept_charset = qq| accept-charset="|;
        if ($S->{UI}->{VARS}->{charsetaccept}) {
                $accept_charset .= $S->{UI}->{VARS}->{charsetaccept} . '"';
        } else {
                $accept_charset = "";
        }

	my $comment_sort = qq|
			<form name="commentmode" action="%%rootdir%%/" method="post"$accept_charset>
	...
	unless ($commentstatus) {
		if ($S->have_perm('comment_post')) {
			if ($S->_check_archivestatus($sid)) {
				...
				$story_info_txt .= qq|\| <a class="post_comment" href="%%rootdir%%/comments/$sid/0/post#here"><b>Post a Comment</b></a> | 

			if ($S->have_section_perm('norm_post_comments',$section) && !$S->_does_poll_exist($sid));

				$story_info_txt .= qq|\| <a class="post_comment" href="%%rootdir%%/comments/poll/$sid/0/post#here"><b>Post a Comment</b></a> | 
			if ($S->_does_poll_exist($sid) && $S->have_perm('poll_post_comments'));
			}
		}
	}
	if (!$S->_does_poll_exist($sid) && $S->{UI}->{VARS}->{allow_story_hide} && ($S->{UID} > 0)) {
		if ($S->_is_hidden($sid)) {
			$story_info_txt .= qq|\| <a class="light" href="%%rootdir%%/unhide/$sid">Un-Ignore Story</a>|;
		} else {
			$story_info_txt .= qq|\| <a class="light" href="%%rootdir%%/hide/$sid">Ignore Story</a>|;
		}
	};
...
sub _set_comment_mode {
	...
	my $thismode = $S->filter_subject($S->cgi->param('commentmode'));
	if ($thismode) {
