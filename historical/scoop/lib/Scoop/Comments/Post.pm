...
sub post_form {
	...
	my $pid = $S->filter_subject($S->{CGI}->param('cid'));
	...
	my $uid = $S->filter_subject($S->{CGI}->param('uid')) || $S->{UID};
	my $pending = $S->filter_subject($S->{CGI}->param('pending'));
	my $user = $S->user_data($uid);
	my $spellcheck = 0;
	if ($preview eq 'Spellcheck') {
		$spellcheck = 1;
	}
...
#                if ($S->have_perm('spellcheck_suggest')) {
#                        $temp = qq| onClick="return (openSpellChecker());" |;
#                };
...
sub filter_comment {
	...
	$comment = $S->ecode_escape($comment, $postmode);
...
sub post_comment {
	my $S = shift;
	my $sid = $S->filter_subject($S->{CGI}->param('sid'));
	my $pid = $S->filter_subject($S->{CGI}->param('cid')) || 0;
	my $subject = $S->{CGI}->param('subject');
	my $comment = $S->{CGI}->param('comment');
	my $uid = $S->filter_subject($S->{CGI}->param('uid')) || $S->{UID};
	my $posttype = $S->filter_subject($S->{CGI}->param('posttype'));
	my $pending = $S->filter_subject($S->{CGI}->param('pending'));
	my $sig_behavior = $S->filter_subject($S->{CGI}->param('sig_behavior'));# || 'retroactive';
	my $sig_status;
	## This is for tracking comments by IP
	my $commentip;
	if ($S->{UI}->{VARS}->{comment_ip_log}) {
		$commentip = $S->{REMOTE_IP} unless ($S->have_perm('no_log_ip'));
	}
