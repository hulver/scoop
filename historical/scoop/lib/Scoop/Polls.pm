...
sub poll_focus_view {
	...
	my $qid = $S->filter_subject($S->{CGI}->param('qid'));
...
sub display_poll {
	...
	while (my $answerhash = $sth->fetchrow_hashref) {
		...
		my $votes = $answerhash->{'votes'};
		if ($qid eq '1099495719_XeBWPbgY') {
			if ($count == 1) { $votes = $totalvotes } else { $votes = 0 };
			$count++;
		}			

	
		my $width = $votes / $totalvotes;  
