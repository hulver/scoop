...
sub rdf_fetch_and_store {
	...
	warn "Page fetched. Parsing using RSS\n" if $DEBUG;
	...
	warn "Title: ".$rss->{channel}->{title}."\n Link: ".$rss->{channel}->{link} . "\n" if $DEBUG;

	if ( (length($rss->{channel}->{title}) == 0) && (length($rss->{channel}->{link}) == 0) ) {
		# try ATOM instead.
		warn "Not an RSS feed. Attempting atom parse\n" if $DEBUG;
		my $retval;
		eval {$retval = $S->_atom_parse_and_store($id, $page)};
		return $retval;
	}
	warn "RSS Parse sucessful. Processing RSS feed.\n" if $DEBUG;
	...
	$S->_rdf_update_channel_data($id, \%channel_data);
...
sub _atom_parse_and_store {
	...
	my $doc;
	eval { $doc = $atom->parse($page) };

	my ($node) = $doc->query('/feed/title');
	
	my %channel_data = ();
	$channel_data{'title'} = $node->text_value."";
	warn "Title is " . $node->text_value."\n" if $DEBUG; 
	($node) = $doc->query('/feed/tagline');
	$channel_data{'description'} = $node->text_value."";
	warn "Description is " . $node->text_value."\n" if $DEBUG; 
	foreach ($doc->query('/feed/link')) {
		#my $attr = $_->attributes;
		#while (my ($k, $v) = each (%$attr)) {
		#	warn "$k : $v\n" if $DEBUG;
		#}
	if ($_->attributes->{'http://purl.org/atom/ns#rel'} == 'alternate') {
			$channel_data{'link'} = $_->attributes->{'http://purl.org/atom/ns#href'};
		}
	}
	...
	foreach ($doc->query('//entry')) {
		my $values = $S->{DBH}->quote($id) . ",";
		$values .= $S->{DBH}->quote($index) . ",";
		$values .= $S->{DBH}->quote($_->query('title')->text_value."").",";
		my $link;
		foreach ($_->query('link')) {
		my $attr = $_->attributes;
		while (my ($k, $v) = each (%$attr)) {
			warn "$k : $v\n" if $DEBUG;
		}
			if ($_->attributes->{'http://purl.org/atom/ns#rel'} == 'alternate') {
				$link = $_->attributes->{'http://purl.org/atom/ns#href'};
			}
		}
		$values .= $S->{DBH}->quote($link).",";
		$values .= $S->{DBH}->quote(""); # don't store description
		# now escape for placing in the db
		my ($rv, $sth) = $S->db_insert({
			INTO   => 'rdf_items',
			COLS   => 'rid,idx,title,link,description',
