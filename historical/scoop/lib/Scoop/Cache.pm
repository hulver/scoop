...
sub fetch {
	...
	if ($data->{compressed}) {
		...
		if ($status != Z_OK) {
			warn "CACHE WARNING: ZLib inflatInit failed with code $status\n";
			return;
			($retval, $status) = $i->inflate($data->{value});
			if ($status != Z_OK) {
				warn "CACHE WARNING: ZLib inflate failed with code $status\n";
				return;
			}
			$retval = Storable::thaw($retval);
		}
	} else {
		$retval = $data->{value};
	}
...
sub store {
	...
	my ($comp_value, $compressed);
	$compressed = $self->{scoop}->{UI}->{VARS}->{compress_cache};
	
	my $len = $self->_calculate_size($value);

	$compressed = 0 if ($len <= 60); # don't compress very small items.

	if ($compressed) {
		my ($d, $status) = deflateInit();
		if ($status == Z_OK) {
			my $temp = Storable::freeze($value);
			($comp_value, $status) = $d->deflate($temp);
			if ($status == Z_OK) {
				if ($len < length($comp_value)) {
					warn "[cache] (store) Compressed, but not stored as no advantage gained\n" if $DEBUG;
					$compressed = 0;
					$comp_value = 0;
				} else {
					warn "[cache] (store) Compressed and stored in cache.\n" if $DEBUG;
					$value = $comp_value;
					$len = length($value);
				}
			} else {
				warn "CACHE WARNING: ZLib deflate failed with code $status\n";
				$compressed = 0;
			}
		} else {
			warn "CACHE WARNING: ZLib defaltInit failed with code $status\n";
			$compressed = 0;
		}
	}
