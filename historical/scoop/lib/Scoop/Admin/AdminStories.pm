...
sub edit_story {
	...
	if ($preview eq 'Spellcheck' && $S->spellcheck_enabled()) {
...
sub save_story {
	...
	if ( !$S->have_perm('select_commentstatus') ) { 
	...
	if ($sid && $sid ne '') {
		# run story update BEFORE we update the story
		# so we can log the prior state of the story.
...
edited because it is currently in $story_mode mode.</B></P>"; }
	...
	if ($mode eq 'full') {
 	...
 	if ($S->spellcheck_enabled()) {
                ...
                $spellbutton = qq|
                        <input type="submit" name="preview" value="Spellcheck"$temp>&nbsp;
			|;
 	};
	if ($mode eq 'full') {
...
#	$story_data->{title} =~ s/"/&quot;/g;
	...
	if ($mode ne 'full') {
		my $section = $form_values->{section};
		my $get_block = lc($section) . "_guidelines";
	
		my $guidelines = $S->{UI}->{BLOCKS}->{$get_block};
		$guidelines = $S->{UI}->{BLOCKS}->{submission_guidelines} unless ($guidelines);
		$form_values->{guidelines} = $guidelines;
	}
