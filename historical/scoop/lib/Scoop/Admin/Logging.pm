...
sub log_activity {
	...
	if ($log_type eq 'comment_delete') {
		...
		$S->_log_story_update ($log_type, $arg1);
	} elsif ($log_type eq 'trackback_delete') {
		$S->_log_trackback_delete ($log_type, $arg1);
	} else {
		...
		<tr>
		  <td>
			...
			<ul>
		      ...
		      <li><a href="%%rootdir%%/admin/log/mail_password">View list of forgotten passwords</a></li>
		      <li><a href="%%rootdir%%/admin/log/invalid_login">View list of invalid logins</a></li>
		      <li><a href="%%rootdir%%/admin/log/erase_user">View list of erased users</a></li>
		    </ul>
		    <b>Actions:</b>
		    <ul>|;
	$content .= qq|
...
sub _log_story_update {
	...
	my ($rv, $sth) = $S->db_select({
		WHAT  => 'aid, title, introtext, bodytext, displaystatus, time',
		FROM  => 'stories',
		WHERE => "sid='$sid'"
	});
	...
	$S->_log_insert($log_type, "$sid", "Author: $aid; Title: $title; Status: $status; Original time: $time", $story);
	$sth->finish;	
...
sub _log_comment_delete {
	...
	$S->_log_insert($log_type, "$sid#$cid", "Author: $author; Original Time: $date; Subject: $subject", $comment);
	$sth->finish;
}

sub _log_trackback_delete {
	my $S    = shift;
	my $log_type = shift;
	my $id  = shift;
	
	my ($rv, $sth) = $S->db_select({
		WHAT  => 'time, sid, url, blog_name, title, excerpt',
		FROM  => 'trackback',
		WHERE => "track_id=?",
		PARAMS => [$id]
	});
	...
	$text .= "Time : " . $r->{time} . "\n";
	$text .= "Url : " . $r->{url} . "\n";
	$text .= "Blog_name : " . $r->{blog_name} . "\n";
	$text .= "Title : " . $r->{title} . "\n";
	$text .= "Excerpt : " . $r->{excerpt} . "\n";

	$text = "" unless ($S->var('use_logging') == 2);

	$S->_log_insert($log_type, "$sid - $id", $r->{url}, $text);
