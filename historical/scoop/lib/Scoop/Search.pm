...
sub search {
	...
	my $disp_count;
	my $result_list;
	my $corrected_count = -1;

	if ($args->{type} ne 'default') {
		...
		($result_list, $corrected_count) = $S->_format_search($args, $sth, $i, $stop);
		$sth->finish;
	}
...
sub _search_form {
	...
	my $search_type = $S->filter_subject($args->{type} || 'default');

	my $story_view = $S->{UI}->{BLOCKS}->{search_story_view};
	my $replace = ($S->{CGI}->param('story_view') ? 'CHECKED' : '' );
	$story_view =~ s/%%CHECKED%%/$replace/g;

	my $phrasebox;
	if($S->{UI}->{VARS}->{use_fulltext_indexes}){
		$phrasebox = $S->{UI}->{BLOCKS}->{fulltext_asphrase};
		$replace = ($args->{phrase})?'CHECKED':'';
		$phrasebox =~ s/%%CHECKED%%/$replace/g;
	}
	my $res_num = {};
	...
	my $last_page_button = '&nbsp;';
	my $next_page_button = '&nbsp;';
	$last_page_button = qq|<INPUT TYPE="submit" NAME="last" VALUE="&lt;&lt; Last Page">| if ($last_page >= 1);
	$next_page_button = qq|<INPUT TYPE="submit" NAME="next" VALUE="Next Page &gt;&gt;">| if ($next_page);
	
	my $page_buttons = $S->{UI}->{BLOCKS}->{search_page_buttons};
	$page_buttons =~ s/%%LAST_PAGE_BUTTON%%/$last_page_button/g;
	$page_buttons =~ s/%%NEXT_PAGE_BUTTON%%/$next_page_button/g;

	my $search_archive = '';
	
	if ($S->{HAVE_ARCHIVE} && ($S->{UI}->{VARS}->{story_archive_age} > 0)) {
		$search_archive = $S->{UI}->{BLOCKS}->{search_archive};
		$replace = ($S->{CGI}->param('search_archive') ? 'CHECKED' : '' );
		$search_archive =~ s/%%CHECKED%%/$replace/g;
	}

	my $hidden_view = '';
	
	if ($S->{TRUSTLEV} == 2 || $S->have_perm('super_mojo')) {
		$hidden_view = $S->{UI}->{BLOCKS}->{search_hidden};
		$replace = ($S->{CGI}->param('hidden_comments') ? 'CHECKED' : '' );
		$hidden_view =~ s/%%CHECKED%%/$replace/g;
				  
	}

	my $form_name = 'search_form_' . $search_type;
	my $form = $S->{UI}->{BLOCKS}->{$form_name};
	
	$form = $S->{UI}->{BLOCKS}->{search_form_default} unless ($form);

	$form =~ s/%%OFFSET%%/$args->{offset}/g;
	$form =~ s/%%COUNT%%/$args->{count}/g;
	$form =~ s/%%SEARCH_TYPE%%/$search_type/g;
	$form =~ s/%%SEARCH_TYPE_SELECT%%/$search_type_select/g;
	$form =~ s/%%TOPIC_SELECT%%/$topic_select/g;
	$form =~ s/%%SECTION_SELECT%%/$section_select/g;
	$form =~ s/%%SEARCH_STRING%%/$args->{string}/g;
	$form =~ s/%%PHRASE_SEARCH%%/$phrasebox/g;
	$form =~ s/%%SEARCH_ARCHIVE%%/$search_archive/g;
	$form =~ s/%%RESULT_SELECT%%/$result_select/g;
	$form =~ s/%%STORY_VIEW%%/$story_view/g;
	$form =~ s/%%HIDDEN_VIEW%%/$hidden_view/g;
	$form =~ s/%%RELEVANCE_RADIO%%/$relevance_radio/g;
	$form =~ s/%%PAGE_BUTTONS%%/$page_buttons/g;
	$form =~ s/%%RESULT_COUNT%%/$disp_count/g;
...
sub search_section_select {
	...
	$list =~ s/<OPTION VALUE="$section">/<OPTION VALUE="$section" SELECTED>/i;
...
sub _determine_search_q {
	...
	if ($args->{type} eq 'user') {
		...
		if ($args->{string}) {
			if ($S->index_search_enabled()) {
				$query->{FROM} .= "inner join sid_xref on c1.sid = sid_xref.sid inner join search_table on search_table.sid_id = sid_xref.sid_id and search_table.cid = c1.cid inner join search_words on search_table.word_id = search_words.word_id LEFT JOIN users u on c1.uid = u.uid";
				my %words = $S->search_words($args->{string});
				my $where;
				my $wordcount = 0;
				...
				if ($wordcount > 1) {
					$query->{WHERE} = "search_words.word in ($where)";
					$query->{WHAT} .= qq|, (COUNT(c1.sid) / $wordcount) * 100 AS matches|;
					$query->{ORDER_BY} = 'matches desc, c1.date desc';
				} else {
#					$where = "''" unless ($where);
					$query->{WHERE} = "search_words.word = $where";
					$query->{WHAT} .= qq|, 100 AS matches|;
					$query->{ORDER_BY} = 'c1.date desc';
				}
			} else {
				#$query->{FROM} .= ' LEFT JOIN comments AS c2 ON c1.cid=c2.pid AND c1.sid=c2.sid';
				#$query->{WHAT} .= qq|, COUNT(c2.pid) AS replies|;
	...
	} elsif (($args->{type} eq 'author') || ($args->{type} eq 'diary_by')) {
		$query->{WHERE} = qq|displaystatus >= 0 $excl_sect_sql|;
		if (($args->{type} eq 'diary_by') || ($args->{section} eq 'Diary')) {
		...
		$query->{ARCHIVE} = ($args->{search_archive} ? 1 : 0);
		$query->{WHAT} = qq|*, $date_format AS ftime, u.nickname AS nick|;
		$query->{WHERE} = "displaystatus >= 0 $excl_sect_sql";
		if ($S->index_search_enabled()) {
			$query->{FROM} = "(stories s inner join sid_xref on s.sid = sid_xref.sid inner join search_table on search_table.sid_id = sid_xref.sid_id inner join search_words on search_table.word_id = search_words.word_id) LEFT JOIN users u on s.aid = u.uid";
			my $wordcount = 0;
			...
			if ($wordcount > 1) {
				$query->{WHERE} .= "and search_words.word in ($where)";
				$query->{WHAT} .= qq|, (COUNT(s.sid) / $wordcount) * 100 AS matches|;
				$query->{ORDER_BY} = 'matches desc, time desc';
			} else {
				$query->{WHERE} .= "and search_words.word = $where";
				$query->{WHAT} .= qq|, 100 AS matches|;
				$query->{ORDER_BY} = 'time desc';
			}
			$query->{WHERE} .= " and search_table.cid = 0";
		} else {
			$query->{FROM} = "stories s LEFT JOIN users u ON s.aid = u.uid";
	                ...
	                if($S->{UI}->{VARS}->{use_fulltext_indexes} && $args->{string} && !$args->{phrase}){
	                        $query->{WHERE} .= qq| AND MATCH(title,introtext,bodytext) AGAINST('$search_str') |;
	                } else{
	                        $query->{WHERE} .= qq| AND (introtext LIKE '%$search_str%' OR bodytext LIKE '%$search_str%' OR title LIKE '%$search_str%') |;
		...
		$query->{GROUP_BY} = qq|s.sid|;


		if ($args->{type} eq 'diary') { 
			# need to put check here that topics are allowed for diaries.
			$args->{section} = 'Diary';
		}
		if ($args->{topic}) {
...
sub _format_story_search {
	...
	while ((my $story = $sth->fetchrow_hashref) && ($i <= $stop)) 
	{
	    ...
	    if( $S->{CGI}->param('story_view') ne "long" ) {
			...
			if ($story->{matches}) {
				$list .= qq| <BR> Matches: <b>| . sprintf( "%.2f", $story->{matches} ) . "%</b>";
			}
