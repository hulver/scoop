...
sub handler {
	...
	# Check if we need to re-direct for a user host name
	my $redirect_to = 0;
	if ($S->{UI}->{VARS}->{allow_user_host}) {
		#my $host = $S->{APACHE}->hostname;
		my $host = $S->{APACHE}->headers_in->{'X-Forwarded-Host'};
		if ($S->param->{host}) {
			$host = $S->param->{host};
		} else {
		...
		if (lc($host) ne lc($S->{UI}->{VARS}->{default_host})) {
			# we want to re-direct to the default host.
			my $new_host = $S->{UI}->{VARS}->{default_host} . "." . $S->{UI}->{VARS}->{default_domain};
			$new_host .= $S->{APACHE}->uri;
			if ($S->{APACHE}->args) {
				$new_host .= '?' . $S->{APACHE}->args;
			};
			$redirect_to = "http://" . $new_host;
			warn "Redirect to $redirect_to" if ($DEBUG);
		} 
	}

	#my $redirect_to = 0; #REMOVE THIS
	unless ($redirect_to) {
	...
	} else { # unless ($redirect_to)
		warn "Redirect to $redirect_to" if ($DEBUG);
		$S->{APACHE}->headers_out->{'Location'} = "$redirect_to";
	}
...
sub _parse_path {
	...
	#unless ($pinfo || $S->{UI}->{VARS}->{main_op_eval}) {
	...
	#$S->param->{op} = (shift(@path) || 'main') unless ($S->param->{op});
