...
sub frontpage_view {
	...
	if( $section && $section ne ''		&&
		...
		my $outsection = $S->filter_subject($section);
		if( $S->have_section_perm( 'deny_read_stories', $section ) ) {
			return qq|<b>%%norm_font%%Sorry, you don't have permission to read stories posted to section '$outsection'.%%norm_font_end%%</b>|;
		} else {
			return qq|<b>%%norm_font%%Sorry, I can't seem to find section '$outsection'.%%norm_font_end%%</b>|;
		}
	...
	if ($section eq 'Diary' && $user) {
		if ($S->{UI}->{VARS}->{allow_user_host}) {
			...
			$pre_link = "http://$newnick%%rootdir%%/main";
		} else {
			$pre_link = "%%rootdir%%/user/$user/diary";
		}
	} elsif ($op eq 'tag') {
		$pre_link = "%%rootdir%%/tag/$tag";
	}
...
sub unhide_story {
	...
	if ($S->{UI}->{VARS}->{allow_story_hide}) {
		...
		my ($rv, $sth) = $S->db_select({
			WHAT => 'hide',
			FROM => 'viewed_stories',
			WHERE => qq|sid = $qsid and uid=$S->{UID}|
			});
		if ($rv) {
			$hide = $sth->fetchrow();
			($rv, $sth) = $S->db_update({
...
sub hide_story {
	...
	if ($S->{UI}->{VARS}->{allow_story_hide}) {
		...
		if ($rv) {
			...
			($rv, $sth) = $S->db_insert({
				INTO => 'viewed_stories',
				COLUMNS => 'sid, uid, hide',
				WHAT => qq|$qsid, $S->{UID}, 1|});
		}
	}
