...
sub _process_macro {
	...
	if ($args) {
		...
		$args =~ s/%%//gs;
		$args =~ s/\n/ /gs;
#		$args = $S->filter_subject($args);
		$args =~ s/,/&#44;/gs;  #Take commas out so not break interpolate
		$args =~ s/%%/&#37;&#37;/gs;
		if (index($args, '[') == 0) {
			$args =~ s/\[//gs;
			@args = split (']', $args);
	...
	warn "Processing macro going to interpolate:\n$macro_text\n" if $DEBUG;
	$macro_text = $S->interpolate($macro_text,$S->{UI}->{BLOCKS},{special =>'true'});
	warn "Results of first pass:\n$macro_text\n" if $DEBUG;
	$macro_text = $S->interpolate($macro_text,$S->{UI}->{VARS},{clear => 'true'});
	warn "Results of second pass:\n$macro_text\n" if $DEBUG;
	$macro_text =~ s/&#44;/,/gs; # put commas back in again
	warn "Inserted commas, final result:\n$macro_text\n" if $DEBUG;
